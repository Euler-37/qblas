subroutine ohemm(side,uplo,m,n,alpha,a,lda,b,ldb,beta,c,ldc)
...
!  ohemm  performs one of the matrix-matrix operations
  ...
  if (upper) then
    do  j = 1,n
      do  i = 1,m
        ...
        if (beta == zero) then
          c(i,j) = temp1*real(a(i,i),kind=16) + alpha*temp2
        else
          c(i,j) = beta*c(i,j) + temp1*real(a(i,i),kind=16) + alpha*temp2
        end if
      end do
    end do
  else
...
!        form  c := alpha*b*a + beta*c.
  ...
  do  j = 1,n
    temp1 = alpha*real(a(j,j),kind=16)
    if (beta == zero) then
      do  i = 1,m
        c(i,j) = temp1*b(i,j)
      end do
