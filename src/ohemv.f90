subroutine ohemv(uplo,n,alpha,a,lda,x,incx,beta,y,incy)
...
!  ohemv  performs the matrix-vector  operation
...
!        form  y  when a is stored in upper triangle.
  ...
  if ((incx == 1) .and. (incy == 1)) then
    do  j = 1,n
      ...
      y(j) = y(j) + temp1*real(a(j,j),kind=16) + alpha*temp2
    end do
  else
    jx = kx
    jy = ky
    do  j = 1,n
      ...
      y(jy) = y(jy) + temp1*real(a(j,j),kind=16) + alpha*temp2
      jx = jx + incx
      jy = jy + incy
    end do
  end if
...
!        form  y  when a is stored in lower triangle.
  ...
  if ((incx == 1) .and. (incy == 1)) then
    do  j = 1,n
      temp1 = alpha*x(j)
      temp2 = zero
      y(j) = y(j) + temp1*real(a(j,j),kind=16)
      do  i = j + 1,n
        y(i) = y(i) + temp1*a(i,j)
        temp2 = temp2 + conjg(a(i,j))*x(i)
      end do
    ...
    do  j = 1,n
      temp1 = alpha*x(jx)
      temp2 = zero
      y(jy) = y(jy) + temp1*real(a(j,j),kind=16)
      ix = jx
      iy = jy
      do  i = j + 1,n
        ix = ix + incx
